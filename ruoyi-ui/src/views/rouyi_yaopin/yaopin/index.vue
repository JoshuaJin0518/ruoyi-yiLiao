    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['rouyi_yaopin:yaopin:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['rouyi_yaopin:yaopin:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['rouyi_yaopin:yaopin:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['rouyi_yaopin:yaopin:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="yaopinList" @selection-change="handleSelectionChange">
...
<script>
import { listYaopin, getYaopin, delYaopin, addYaopin, updateYaopin } from "@/api/rouyi_yaopin/yaopin";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listYaopin(this.queryParams).then(response => {
        this.yaopinList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateYaopin(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addYaopin(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('rouyi_yaopin/yaopin/export', {
        ...this.queryParams
      }, `yaopin_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
